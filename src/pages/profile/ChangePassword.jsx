...
function EditProfile() {
  ...
  const validatePassword = (password) => {
    const newErrors = {}
    if (!minLen.test(password)) newErrors.minLen = 'Password must be at least 8 characters'
    if (!hasLower.test(password)) newErrors.lower = 'Must contain a lowercase letter'
    if (!hasUpper.test(password)) newErrors.upper = 'Must contain an uppercase letter'
    if (!hasDigit.test(password)) newErrors.digit = 'Must contain a number'
    if (!hasSymbol.test(password)) newErrors.symbol = 'Must contain a special character'
